...
class MemoryCatalogWorker(QThread):
    """記憶體目錄操作工作執行緒 - 參考Keithley架構"""
    catalog_ready = pyqtSignal(dict)  # memory_catalog
    ...
    def run(self):
        ...
        try:
            ...
            if not self.dp711 or not self.dp711.connected:
                self.error_occurred.emit("設備未連接")
                return
            ...
            for mem_num in range(1, 6):
                try:
                    self.progress_updated.emit(10 + mem_num * 15)
                    
                    # 使用安全的記憶體信息獲取方法
                    mem_info = self._get_memory_info_safe(mem_num)
                    if mem_info:
                        memory_catalog[mem_num] = mem_info
                        
                except Exception as e:
                    # 記錄錯誤但繼續處理其他記憶體
                    print(f"獲取記憶體 {mem_num} 失敗: {e}")
                    continue
            
            self.progress_updated.emit(100)
            self.catalog_ready.emit(memory_catalog)
            
        except Exception as e:
            self.error_occurred.emit(f"獲取記憶體目錄失敗: {e}")
    
    def _get_memory_info_safe(self, mem_num: int) -> Optional[Dict]:
        """安全地獲取記憶體信息"""
        try:
            # 載入記憶體狀態
            if self.dp711.recall_memory_state(mem_num):
                # 等待設備穩定
                self.msleep(200)
                
                # 讀取記憶體中的設定，使用簡化的方法避免超時
                try:
                    mem_voltage = self.dp711.get_set_voltage()
                    mem_current = self.dp711.get_set_current()
...
class RigolControlWidget(QWidget):
    ...
    def create_enhanced_lcd_panel(self):
        ...
        self.output_status_text.setStyleSheet("""
            font-size: 16px; 
            font-weight: bold; 
            color: #e74c3c;
            padding: 5px;
    ...
    def update_output_status_display(self, is_output_on: bool):
        """統一更新輸出狀態顯示"""
        if is_output_on:
            # 輸出開啟狀態
            self.output_status.setStyleSheet("""
                ...
                min-width: 50px;
            """)
            self.output_status_text.setText("開啟")
            self.output_status_text.setStyleSheet("""
                font-size: 16px; 
                font-weight: bold; 
                color: #27ae60;
            ...
            self.output_status.setStyleSheet("""
                ...
                min-width: 50px;
            """)
            self.output_status_text.setText("關閉")
            self.output_status_text.setStyleSheet("""
                font-size: 16px; 
                font-weight: bold; 
                color: #e74c3c;
    ...
    def toggle_output(self):
        ...
        try:
            if self.dp711.is_output_on():
                ...
                self.update_output_status_display(False)
                self.log_message("輸出已關閉")
            else:
                self.dp711.set_output(True)
                self.output_btn.setText("關閉輸出")
                self.update_output_status_display(True)
    ...
    def refresh_memory_catalog_async(self):
        """使用後台線程刷新記憶體內容目錄，避免GUI凍結"""
        if not self.dp711:
            return
            
        if hasattr(self, 'memory_worker') and self.memory_worker.isRunning():
            self.log_message("記憶體目錄更新已在進行中...")
            return
        
        # 創建記憶體操作worker
        self.memory_worker = MemoryCatalogWorker(self.dp711)
        
        # 連接信號
        self.memory_worker.catalog_ready.connect(self.on_memory_catalog_ready)
        self.memory_worker.progress_updated.connect(self.on_memory_progress)
        self.memory_worker.error_occurred.connect(self.on_memory_error)
        self.memory_worker.finished.connect(self.on_memory_finished)
        
        # 更新UI狀態
        if hasattr(self, 'refresh_memory_btn'):
            self.refresh_memory_btn.setText("載入中...")
            self.refresh_memory_btn.setEnabled(False)
        
        self.log_message("開始載入記憶體目錄...")
        
        # 啟動worker
        self.memory_worker.start()
    
    def on_memory_catalog_ready(self, memory_catalog):
        """記憶體目錄準備就緒的回調"""
        try:
            # 更新記憶體選擇器顯示
            for i, (mem_num, mem_info) in enumerate(memory_catalog.items()):
                if 'error' not in mem_info:
                    ...
                    if i < len(self.quick_memory_btns):
                        self.quick_memory_btns[i].setToolTip(display_text)
                        self.quick_memory_btns[i].setEnabled(True)
            
            self.log_message("記憶體目錄已刷新")
            
        except Exception as e:
            self.logger.error(f"更新記憶體顯示時發生錯誤: {e}")
    
    def on_memory_progress(self, progress):
        """記憶體操作進度更新"""
        if hasattr(self, 'refresh_memory_btn'):
            self.refresh_memory_btn.setText(f"載入中 {progress}%")
    
    def on_memory_error(self, error_msg):
        """記憶體操作錯誤"""
        self.log_message(f"記憶體操作錯誤: {error_msg}")
        
    def on_memory_finished(self):
        """記憶體操作完成"""
        if hasattr(self, 'refresh_memory_btn'):
    ...
    def refresh_memory_catalog(self):
        """刷新記憶體內容目錄 - 保持向後相容"""
        # 使用異步版本
        self.refresh_memory_catalog_async()
    ...
    def update_device_controls(self):
        ...
        if self.dp711:
            try:
                ...
                # 設置記憶體組合框變化監聽
                if not hasattr(self, '_memory_combo_connected'):
                    self.memory_combo.currentIndexChanged.connect(self._update_memory_preview)
                    self._memory_combo_connected = True
    ...
    def reset_device(self):
        """重置設備到出廠預設狀態"""
        if not self.dp711 or not self.dp711.is_connected():
            QMessageBox.warning(self, "警告", "沒有連接的設備")
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                # 首先關閉輸出
                if self.dp711.is_output_enabled():
                    self.dp711.output_off()
                    
                # 發送重置指令
                self.dp711._send_command("*RST")
                
                # 重新初始化設備
                self.dp711._initialize_device()
    ...
    def clear_device_protection(self):
        ...
        try:
            # 清除保護狀態的SCPI指令
            self.dp711._send_command("OUTP:PROT:CLE")
    ...
    def set_track_mode(self, mode_text: str):
        ...
        try:
            ...
            # 發送追蹤模式指令 (如果設備支援)
            self.dp711._send_command(f"OUTP:TRACK {mode}")
    ...
    def refresh_device_status(self):
        ...
        try:
            # 查詢設備溫度 (如果支援)
            try:
                temp_response = self.dp711._query_command("SYST:TEMP?")
                if temp_response:
            ...
            # 查詢保護狀態 (如果支援)
            try:
                prot_response = self.dp711._query_command("SYST:ERR?")
    ...
    def log_message(self, message: str):
        ...
        # 同時記錄到logger，移除特殊Unicode字符避免編碼錯誤
        clean_message = (message.replace("✅", "[OK]")
                        .replace("❌", "[ERROR]")
                        .replace("⚠️", "[WARNING]")
                        .replace("🔄", "[PROCESSING]")
                        .replace("✓", "[SUCCESS]")
